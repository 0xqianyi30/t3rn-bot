...
# 每个链的颜色和符号
chain_symbols = {
    'Arbitrum': '\033[34m',  # 使用与 Base 相同的颜色，或者你可以选择其他颜色
    'OP Sepolia': '\033[91m',         
}
...
# 每个网络的区块浏览器URL
explorer_urls = {
    'Arbitrum': 'https://sepolia.arbiscan.io/tx/',  # Arbitrum Sepolia 区块浏览器
...
def process_network_transactions(network_name, bridges, chain_data, successful_txs):
    ...
    for bridge in bridges:
        for i, private_key in enumerate(private_keys):
            ...
            # 随机等待 60 到 130 秒
            wait_time = random.uniform(60, 130)
...
def main():
    ...
    while True:
        ...
        # 如果余额不足 0.301 ETH，切换到另一个链
        if balance < 0.301:
            print(f"{chain_symbols[current_network]}{current_network}余额不足 0.301 ETH，切换到 {alternate_network}{reset_color}")
            current_network, alternate_network = alternate_network, current_network  # 交换链

        # 处理当前链的交易
        successful_txs = process_network_transactions(
            current_network,
            ["Arbitrum - OP Sepolia"] if current_network == 'Arbitrum' else ["OP - Arbitrum"],
